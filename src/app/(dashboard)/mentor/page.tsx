...
// ============================================================
// RAILREADY — Page Mentor Ferroviaire
// /mentor
// ============================================================
...
export default function MentorPage() {
  ...
  useEffect(() => {
    async function loadWelcome() {
      try {
        ...
        setMessages([{
          role: 'assistant',
          content: 'Bonjour ! Je suis le Mentor Ferroviaire de RailReady. Posez-moi vos questions sur les métiers du rail — je suis là pour vous aider à y voir clair.',
        }])
      } finally {
  ...
  // Questions suggérées
  const suggestions = [
    'Quel métier ferroviaire me correspond ?',
    'C\'est quoi vraiment la vie de conducteur de train ?',
    'Je veux me reconvertir dans le ferroviaire, par où commencer ?',
    'Les horaires décalés, comment ça se passe vraiment ?',
  ]
    ...
    <div className="flex flex-col h-[calc(100vh-80px)] max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Orientation • Réalités terrain • Préparation candidature
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        {messages.map((msg, i) => (
          ...
          >
            {msg.role === 'assistant' && (
              <div className="w-7 h-7 bg-gray-800 rounded-full flex items-center justify-center text-xs flex-shrink-0 mr-2 mt-1">
                🚆
              </div>
            )}
            <div
        ...
        {messages.length === 1 && !isInitializing && (
          <div className="space-y-2 pt-2">
            <p className="text-xs text-gray-400 text-center">Questions suggérées</p>
      ...
      {/* Bouton feedback bêta — apparaît après 5 échanges */}
      {msgCount >= 4 && (
        <div className="px-4 py-2 flex-shrink-0 flex justify-center">
          ...
          >
            🏗️ Donner mon retour terrain sur le Mentor
          </button>
        </div>
      )}
      ...
      <div className="px-4 py-1 flex-shrink-0">
        <p className="text-xs text-gray-400 text-center">
          Le Mentor répond sur les réalités terrain et l'orientation. Il ne fournit pas de procédures techniques ou de contenus d'examen officiels.
        </p>
      </div>
